...
const ProductManagement = () => {
  ...
  return (
    <div className="product-management" style={{ pointerEvents: 'auto' }}>
          ...
          <tbody>
            {products.map(product => (
              <tr key={product.id} className={product.quantity <= (product.lowStockThreshold || 5) ? 'low-stock' : ''}>
                <td>
                  {product.imageUrl && (
                    <img src={product.imageUrl} alt={product.name} style={{ width: '50px', height: '50px', objectFit: 'cover', marginRight: '10px' }} />
                  )}
                  {product.name}
                </td>
